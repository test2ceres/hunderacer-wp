...
TRUNCATE TABLE `hunderacer-wp`.wp_comments;
TRUNCATE TABLE `hunderacer-wp`.wp_links;
TRUNCATE TABLE `hunderacer-wp`.wp_postmeta;
TRUNCATE TABLE `hunderacer-wp`.wp_posts;
TRUNCATE TABLE `hunderacer-wp`.wp_term_relationships;
TRUNCATE TABLE `hunderacer-wp`.wp_term_taxonomy;
TRUNCATE TABLE `hunderacer-wp`.wp_terms;
...
DELETE FROM `hunderacer-wp`.wp_users WHERE ID > 1;
DELETE FROM `hunderacer-wp`.wp_usermeta WHERE user_id > 1;
...
UPDATE `hunderacer-wp`.wp_posts SET post_status = 'pending' WHERE post_type = 'page';
...
# UPDATE `hunderacer-wp`.wp_posts SET post_content = REPLACE(post_content, '"/files/', '"/wp-content/uploads/');
...
# CATEGORIES
# These are NEW categories, not in `hunderacer_dr`.drupal_ Add as many sets as needed.
...
UPDATE `hunderacer-wp`.wp_terms SET name = 'First Category' WHERE name = 'first category';
UPDATE `hunderacer-wp`.wp_terms SET name = 'Second Category' WHERE name = 'second category';
UPDATE `hunderacer-wp`.wp_terms SET name = 'Third Category' WHERE name = 'third category';
...
INSERT IGNORE INTO `hunderacer-wp`.wp_term_relationships (object_id, term_taxonomy_id)
	SELECT DISTINCT p.ID AS object_id,
		(SELECT tt.term_taxonomy_id
		FROM `hunderacer-wp`.wp_term_taxonomy tt
		INNER JOIN `hunderacer-wp`.wp_terms t USING (term_id)
		WHERE t.slug = 'enter-category-slug-here'
		AND tt.taxonomy = 'category') AS term_taxonomy_id
	FROM `hunderacer-wp`.wp_posts p
	WHERE p.post_content LIKE '%enter string to match here%'
	OR p.ID IN (
		SELECT tr.object_id
		FROM `hunderacer-wp`.wp_term_taxonomy tt
		INNER JOIN `hunderacer-wp`.wp_terms t USING (term_id)
		INNER JOIN `hunderacer-wp`.wp_term_relationships tr USING (term_taxonomy_id)
...
# User ID values are carried over from `hunderacer_dr`.drupal_
UPDATE `hunderacer-wp`.wp_usermeta
...
#UPDATE IGNORE `hunderacer-wp`.wp_posts p, `hunderacer_dr`.drupal_content_field_video v
...
UPDATE IGNORE `hunderacer-wp`.wp_posts p, `hunderacer_dr`.drupal_content_field_gallery i, `hunderacer_dr`.drupal_files f
	SET p.post_content =
		CONCAT(
			CONCAT(
				'<div class="drupal_image_wrapper"><img src="wp-content/uploads/files/',
				f.filename,
