...
function EWD_FEUP_Add_WP_User($User_ID, $User_Fields) {
	...
	foreach ($Fields as $Field) {
		if ($Field->Field_Equivalent == "First_Name") {$user_data['first_name'] = $wpdb->get_var($wpdb->prepare("SELECT Field_Value FROM $ewd_feup_user_fields_table_name WHERE Field_ID=%d", $Field->Field_ID));}
		elseif ($Field->Field_Equivalent == "Last_Name") {$user_data['last_name'] = $wpdb->get_var($wpdb->prepare("SELECT Field_Value FROM $ewd_feup_user_fields_table_name WHERE Field_ID=%d", $Field->Field_ID));}
		elseif ($Username_Is_Email != "Yes" and $Field->Field_Equivalent == "Email") {$user_data['user_email'] = $wpdb->get_var($wpdb->prepare("SELECT Field_Value FROM $ewd_feup_user_fields_table_name WHERE Field_ID=%d", $Field->Field_ID));}
	}
